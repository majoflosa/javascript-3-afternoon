...
var employee = new Employee( 'John', 'Doe', 'johndoe@example.com', 35 );
...
class Manager extends Employee {
  constructor() {
    super( 'Dallin', 'Anderson', 'Dallin@DallinDallin.com', 25 );
    this.reports = [];
  }
...
class ProgressiveManager extends Manager {
  ...
  hire( employee ) {
    ...
    if ( this.reports.length === 0 ) {
      this.title = 'Not a manager';
    } else if ( 0 < this.reports.length && this.reports.length <= 3 ) {
      this.title = 'Barely Manager';
    } else if ( 3 < this.reports.length && this.reports.length <= 10 ) {
      this.title = 'Mostly Manager';
    } else if ( 10 < this.reports.length && this.reports.length <= 50 ) {
      this.title = 'Manager';
    } else if ( 50 < this.reports.length && this.reports.length <= 100 ) {
...
class Machine {
  ...
  makeWidgets( num ) {
    ...
    var fiftyCounts = Math.floor(num / 50);
    this.wear_and_tear_count += fiftyCounts;
  }
  ...
  reboot() {
    // var wat = this.wear_and_tear_count
...
console.log( 'before reboot: ', machine.wear_and_tear_count );
// console.log( 'reboot', machine.reboot() );
machine.reboot();
console.log( 'after reboot: ', machine.wear_and_tear_count );
