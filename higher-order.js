...
let ordersTotal = orders.map( order => {
  let orderTotal = order.price + (order.price * order.tax);
  return orderTotal;
})

// console.log( typeof ordersTotal );
...
let bobsTotal = purchases.reduce( (total, purch) => {
...
//   return total += curr.owner === "Bob" ? curr.price : 0;
// }, 0);
